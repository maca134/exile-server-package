...
if(_pincode isEqualTo _objectPinCode)then
{
	...
	[_sessionID,"systemChatRequest",[format ["New pin set to: %1!",_newPinCode]]] call ExileServer_system_network_send_to;
	[_object,_newPinCode] call ExileServer_object_container_database_setpin;
}
else
{
	[_sessionID,"systemChatRequest",["Wrong Pin!"]] call ExileServer_system_network_send_to;
};
true